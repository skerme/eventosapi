...
export class CompraService {
    constructor(
      @InjectModel('Compra') private readonly vendaModel: Model<Compra>,
    ) {}
    ...
    async getByEstado(inicio=new Date() ,  fim=new Date()   ):Promise<Compra[]> {
     ...
     console.log("eeeeeee333333333",intervalo_inicio, intervalo_fim)
      ...
      console.log("eeeeeee33333333355555555555555",intervalo_inicio, intervalo_fim)
  
  
      return await this.vendaModel.find( {"$and": [  {estado:1},  {dataDaVenda:{$gte: intervalo_inicio, $lt: intervalo_fim }}  ] }   ).sort({codigo:-1}).exec();
    }
  ...
  console.log("resultado",resultado )
    ...
    async update(id: string,  compra: Compra): Promise<Compra> {
      await this.vendaModel.findByIdAndUpdate({ _id: id },  compra).exec();
    ...
    async getById(id: string) {
      return await this.vendaModel.findById(id).exec();
    }
  ...
  var totalvendas=await this.vendaModel.find({dataDaVenda:{   $gte: intervalo_inicio,$lt: intervalo_fim }}).count()
